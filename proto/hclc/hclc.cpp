...
json update_chain(ConnCtx *c, json cont = {}) {
  /** aliasing */
...
// open contact - COPEN
json client_open(ConnCtx *c, std::string chain_trip) {
    try {
      /** aliasing */  
      auto eCTX = (c->ExchangeCtx);
      auto gCTX = (c->GraphCtx);      
...
json host_open(ConnCtx *c, json cont) {
  try {
    /** aliasing */
    auto eCTX = (c->ExchangeCtx);
    auto gCTX = (c->GraphCtx);
...
json transfer_blocks(ConnCtx *c, json cont) {
    try {
        auto eCTX = (c->ExchangeCtx);
        auto gCTX = (c->GraphCtx);
        auto TREE = (*(gCTX.ParentMap))[eCTX.ChainTrip];
...
// Keyex
void _Key_Exchange(ConnCtx *c) {
  auto NET = (c->Networking);
  
  json _j;
  _j["FLAG"] = "KE";
  _j["CONT"] = (c->Networking).sec.Public();
...
std::map<std::string /*prev flag*/, json (*)(ConnCtx*, json)> next {
    {"COPEN", &host_open},
