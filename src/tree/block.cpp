...
block::jdump() const {
  ...
  for (const auto& ph : this->p_hashes) v_p_hashes.push_back(ph);

  output["d"] = std::vector<std::string>({
      enc_time, 
      this->nonce, 
      this->s_trip, 
      this->c_trip, 
...
block::block(json input) { 
  ...
  for (const auto& ph: v_p_hashes) this->p_hashes.insert(ph);
}
